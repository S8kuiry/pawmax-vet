...
export default async function BookingsPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  const now = new Date();
  const query: any = { ownerId: session!.id };
  if (tab === "upcoming") query.date = { $gte: now };
  else if (tab === "past") query.status = "completed";
  else if (tab === "cancelled") query.status = "cancelled";

  const bookings = await Booking.find(query).sort({ date: tab === "past" ? -1 : 1 }).lean();
