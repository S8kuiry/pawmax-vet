...
export default async function PrescriptionsPage() {
  ...
  await dbConnect();
  const items = await Prescription.find({ ownerId: session!.id }).sort({ issuedAt: -1 }).lean();
            ...
            <Link key={String(r._id)} href={`/owner/prescriptions/${r._id}`}
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1">Prescribed by {r.vetName} on {new Date(r.issuedAt).toLocaleDateString()}</p>
                </div>
